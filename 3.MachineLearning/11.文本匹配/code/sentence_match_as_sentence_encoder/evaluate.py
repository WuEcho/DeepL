...
class Evaluator:
    ...
    def eval(self, epoch):
        ...
        self.knwb_to_vector()  #将knwb数据转换为向量
        for index, batch_data in enumerate(self.valid_data):
            if torch.cuda.is_available():
                batch_data = [d.cuda() for d in batch_data]  #如果可用GPU，将数据转移到GPU上
            input_id, labels = batch_data   #输入变化时这里需要修改，比如多输入，多输出的情况
            with torch.no_grad():
                test_question_vectors = self.model(input_id) #不输入labels，使用模型当前参数进行预测
            self.write_stats(test_question_vectors, labels)  #将预测结果和真实标签写入统计字典
        self.show_stats()  #显示统计结果
        return
    ...
    def write_stats(self, test_question_vectors, labels):
        ...
        for test_question_vector, label in zip(test_question_vectors, labels):
            ...
            #正常来说应该做的是输入的句子的向量与文本中所有问题句子的向量做相似度计算，然后取最大值
            #这里使用的是矩阵乘法，效率更高
