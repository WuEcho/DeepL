...
A.class_foo()
```
- 在 Python 中，**类方法（`@classmethod`）、静态方法（`@staticmethod`）和实例方法** 各有不同的设计目的和适用场景。以下是它们的核心区别及选择建议：

 **1. 实例方法（`def get_list(self)`）**
 **特点**：
- **参数要求**：第一个参数必须是 `self`（代表类的实例）。
- **访问权限**：可以直接访问实例属性（如 `self.age`）和类属性（通过 `self.__class__.age` 或类名）。
- **作用对象**：操作**实例级别的数据**（如修改实例状态）。
 **使用场景**：
- 需要读写实例的属性（如 `self.age`）。
- 需要依赖实例的状态（如根据实例属性计算结果）。
 **示例**：
```python
def get_list(self):
 ...
 **2. 类方法（`@classmethod`）**
 **特点**：
- **参数要求**：第一个参数必须是 `cls`（代表类本身）。
- **访问权限**：可以直接访问**类属性**（如 `cls.age`），但不能直接访问实例属性（除非通过实例传递）。
- **作用对象**：操作**类级别的数据**（如修改类状态或创建类的实例）。
 **使用场景**：
- 需要操作类属性（如 `cls.age`）。
- 实现工厂模式（通过类方法创建不同子类的实例）。
- 单例模式中管理类的唯一实例。
 **示例**：
```python
@classmethod
def get_class_list(cls):
    return cls.age  # 访问类属性
```
 **3. 静态方法（`@staticmethod`）**
 **特点**：
- **参数要求**：无需 `self` 或 `cls` 参数。
- **访问权限**：不能直接访问类属性或实例属性，需显式传递。
- **作用对象**：与类相关但**不依赖类或实例状态**的工具函数。
 **使用场景**：
- 提供与类逻辑相关的工具函数（如数据验证、数学计算）。
- 用于代码组织（将相关函数放在类内，但无需访问类/实例数据）。
 **示例**：
```python
@staticmethod
...
| **方法类型**   | **选择依据**                                                                 |
|----------------|----------------------------------------------------------------------------|
| **实例方法**   | 需要操作实例的属性（如 `self.age`）或依赖实例的状态时使用。                     |
| **类方法**     | 需要操作类属性（如 `cls.age`）或管理类级别的行为（如工厂模式、单例模式）时使用。 |
| **静态方法**   | 提供与类相关但无需访问类/实例数据的工具函数时使用。                               |
 **常见错误示例**
 1. **在静态方法中访问类/实例属性**
```python
@staticmethod
def get_static_list():
    return cls.age  # 错误！静态方法无法直接访问 `cls` 或 `self`
```
...
@classmethod
def get_class_list(cls):
    return self.age  # 错误！类方法无法直接访问 `self`
```
...
obj = MyClass()
print(obj.age)          # 输出 10（实例属性）
print(MyClass.age)      # 输出 0（类属性）
```

### **总结**
- **优先使用实例方法**：当需要操作实例数据时（90% 的场景）。
- **谨慎使用类方法**：仅在需要操作类属性或管理类级别行为时使用。
- **合理使用静态方法**：将工具函数放在类内，但确保它们不依赖类/实例状态。
